      ...
      <n-card class="card">
        ...
        <div class="content" v-else-if="query.result.value?.user">
            ...
            <div class="about-wrapper">
              <n-grid cols="1 350:3 650:5" class="grid">
                ...
                <n-grid-item>
                  ...
                  <p>
                    <RelativeTimeTooltip
                      :date="new Date(query.result.value.user.joined)"
                    />
                  </p>
                </n-grid-item>
                <n-grid-item>
                  ...
                  <p v-if="query.result.value.user.accessLogs?.length > 0">
                    <RelativeTimeTooltip
                      :date="
                        new Date(
                          query.result.value.user.accessLogs?.[0]?.timestamp
                        )
                      "
                    />
                  </p>
                  <p v-else class="italic">Never</p>
                </n-grid-item>
                <n-grid-item>
          ...
          >
            ...
            <n-grid-item
              v-if="ability.can(AbilityActions.Update, userSubject, 'password')"
            >
              <RouterLink
                :to="{
...
const query = useQuery(UserDetailsDocument, { id: props.userId });
...
function deleteUser() {
  dialog.error({
    ...
    onPositiveClick: async () => {
      await deleteMutation.mutate({ id: props.userId });
